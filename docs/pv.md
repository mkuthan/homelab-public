...
- The home is connected to the inverter's backup output.
  In case of inverter failure, the home can be powered directly from the grid by switching the "Photovoltaic - Grid selector".
- The heat pump is powered directly from the grid.
  The heat accumulated by the house is sufficient to survive day-length power outages.

```
      Main switch                        Photovoltaic - Grid selector
GRID o----/  ----+-----------------------------------o
                 |                                     
                 | Inverter switch    +----------+      o------o HOME 
                 +-------/  ----------| Inverter |---o /
                 |                    +----------+ 
                 |
                 | Heatpump switch    +----------+
